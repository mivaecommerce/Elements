...
(function ($, window, document) {
	...
	var $overflowNavigation = $.hook('x-overflow-navigation'),
		$displayButton = $.hook('x-overflow-navigation__trigger'),
		$visibleLinks = $.hook('x-overflow-navigation__row'),
		$hiddenLinks = $.hook('x-overflow-navigation__group'),
		numOfItems = 0,
		totalSpace = 0,
		closingTime = 1000,
		breakWidths = [],
	...
	/*$.hook('has-child').on('mouseenter', function (e) {
		var parentElement = $(this),
			childElement = parentElement.children('ul'),
			childOffsetWidth = childElement.offset().left + childElement.width(),
			documentWidth = document.documentElement.clientWidth,
			isEntirelyVisible = (childOffsetWidth <= documentWidth);

		if (!isEntirelyVisible) {
			parentElement.addClass('edge');
		}
		else {
			parentElement.removeClass('edge');
		}
	});*/
	...
	$.fn.doubleTapToGo = function() {
		if (!sessionStorage.getItem('USER_IS_TOUCHING') && sessionStorage.getItem('USER_IS_TOUCHING') !== 'true') {
			return false;
		}
	...
	$.fn.setOffsetDirection = function (container) {
		...
		triggerElement.on('mouseenter', function (e) {
			var parentElement = $(this),
				childElement = parentElement.find('ul'),
				grandchildElement = childElement.find('ul'),
				childOffsetWidth = childElement.offset().left + childElement.width(),
	...
	$.fn.overflowNavigation = function (container) {
		...
		$('body').on('click', function (event) {
			if ($(event.target).closest($.hook('x-overflow-navigation')).length === 0) {
				$hiddenLinks.addClass('is-hidden');
			}
		});
